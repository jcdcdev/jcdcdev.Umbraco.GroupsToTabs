# Groups to Tabs

[![Documentation](https://img.shields.io/badge/Documentation-123?color=394933&logo=data:image/svg+xml;base64,&style=for-the-badge)](https://docs.jcdc.dev/jcdcdev-umbraco-groupstotabs/latest)
[![Umbraco Marketplace](https://img.shields.io/badge/Umbraco%20Marketplace-%23f5c1bc?logo=umbraco&logoColor=162335&style=for-the-badge)](https://marketplace.umbraco.com/package/jcdcdev.Umbraco.GroupsToTabs)
[![GitHub](https://img.shields.io/badge/GitHub-1?logo=github&color=232925&style=for-the-badge)](https://github.com/jcdcdev/jcdcdev.Umbraco.GroupsToTabs)
[![NuGet Downloads](https://img.shields.io/nuget/dt/jcdcdev.Umbraco.GroupsToTabs?labelColor=4536d3&color=4536d3&label=NuGet&logo=nuget&style=for-the-badge)](https://www.nuget.org/packages/jcdcdev.Umbraco.GroupsToTabs)
[![Project Website](https://img.shields.io/badge/Project%20Website-jcdcdev?color=3c4834&logo=data:image/svg+xml;base64,&style=for-the-badge)](https://jcdc.dev/umbraco-packages/groups-to-tabs)


A simple Umbraco package that converts content type groups into tabs in the Umbraco backoffice.

> [!NOTE]
> This package was created to celebrate [Hacktoberfest](https://umbraco.com/blog/join-us-for-umbraco-hacktoberfest-2024/) 2024! 🎉

## Security

This project takes security and support seriously.
Please visit the [Security](https://github.com/jcdcdev/jcdcdev.Umbraco.GroupsToTabs?tab=security-ov-file) page for more information.

## Contributing

Contributions to this package are most welcome! Please visit the [Contributing](https://github.com/jcdcdev/jcdcdev.Umbraco.GroupsToTabs/contribute) page.

## Acknowledgements

Thank you to the following projects and individuals for their contributions. High five, you rock! 🤘🦄

- LottePitcher - [opinionated-package-starter](https://github.com/LottePitcher/opinionated-package-starter)
- jcdcdev - [jcdcdev.Umbraco.PackageTemplate](https://github.com/jcdcdev/jcdcdev.Umbraco.PackageTemplate)



